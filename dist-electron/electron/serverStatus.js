...
exports.getSupportedMinecraftVersions = getSupportedMinecraftVersions;
exports.pingServer = pingServer;
exports.resolveServerTarget = resolveServerTarget;
const minecraft_protocol_1 = require("minecraft-protocol");
const promises_1 = require("node:dns/promises");
function getSupportedMinecraftVersions() {
    return [...minecraft_protocol_1.supportedVersions]
        .filter((version) => /^\d+\.\d+(\.\d+)?$/.test(version))
        .sort(compareMinecraftVersions);
}
async function pingServer(target) {
...
async function resolveServerTarget(target) {
    const parsed = parseServerAddress(target.host, target.port);
    const baseTarget = {
        host: parsed.host,
        port: parsed.port,
    ...
    try {
        const records = await (0, promises_1.resolveSrv)(`_minecraft._tcp.${parsed.host}`);
...
async function pingWithFallback(target) {
    const baseOptions = {
        host: target.host,
        port: target.port,
        closeTimeout: 8_000,
    ...
    try {
        return await (0, minecraft_protocol_1.ping)({ ...baseOptions, version: target.version });
    }
    catch (firstError) {
        try {
            return await (0, minecraft_protocol_1.ping)(baseOptions);
        }
        catch {
            throw firstError;
        }
...
function parseServerAddress(hostInput, fallbackPort) {
...
function isNewPingResult(value) {
...
function textFromComponent(value) {
    ...
    if (isRecord(value)) {
        ...
        const translated = ownText.length === 0 && typeof value.translate === "string" ? value.translate : "";
...
function readPlayerCount(value, kind) {
    if (kind === "online") {
        return (readOptionalNumber(value.playerCount) ??
            readOptionalNumber(value.onlinePlayers) ??
            readOptionalNumber(value.online) ??
            readNestedNumber(value, ["players", "online"]));
    }
    return (readOptionalNumber(value.maxPlayers) ??
